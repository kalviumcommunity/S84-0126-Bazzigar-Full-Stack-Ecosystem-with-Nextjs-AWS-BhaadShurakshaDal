...
const features = [
  ...
  {
    icon: <Bell className="w-8 h-8 text-red-600 dark:text-red-400" />,
    ...
    <section className="py-24 relative overflow-hidden">
      <div className="container px-4 md:px-6 mx-auto">
        <div className="text-center mb-16">
          ...
          <p className="text-slate-600 dark:text-zinc-400 max-w-2xl mx-auto">Our technology saves lives by bridging the gap between data and action.</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {features.map((feature, index) => (
            <motion.div
              ...
              className="glass p-8 rounded-3xl hover:bg-white/90 dark:hover:bg-white/10 transition-all hover:-translate-y-2 cursor-pointer group border border-slate-200 dark:border-transparent shadow-sm hover:shadow-lg"
            >
              <div className="mb-6 p-4 rounded-2xl bg-gradient-to-br from-blue-50 to-cyan-50 dark:bg-white/5 w-fit group-hover:scale-110 transition-transform duration-300">
                {feature.icon}
              </div>
              <h3 className="text-xl font-bold mb-3 text-slate-900 dark:text-white">{feature.title}</h3>
              <p className="text-slate-600 dark:text-zinc-400 leading-relaxed text-sm">{feature.description}</p>
            </motion.div>
          ))}
        </div>
      </div>
